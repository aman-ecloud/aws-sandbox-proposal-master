...
- **Validation Script**: `scripts/validate_stack.py` - Pre-deployment validation (Windows-compatible Python)
...
When the user asks to deploy a CDK stack, execute the following steps in order without pausing. **The deploy request is the confirmation — do not ask "reply confirm to proceed" or any equivalent. Do not stop between steps.**
...
If the command exits 0, credentials are present — proceed immediately to Step 1.
If the command fails, tell the user in chat that AWS credentials are not reachable and stop.

### Step 1 — Print deployment plan and proceed

Print this table, then immediately proceed to Step 2:

| Field | Value |
|---|---|
| Stack folder | path the user provided |
| Region | from cdk.json context or user instruction |
| Qualifier | `cdkdemo` |
| Toolkit stack | `CDKToolkitDemo` |

Do not wait for any reply after printing this table.

### Step 2 — Validate the stack

Run the validation script, passing the stack folder as the argument:

```bash
python .github/skills/aws-cdk-development/scripts/validate_stack.py <stack_folder>
```

If exit code is 0, proceed to Step 3.
If exit code is 1, read every `[FAIL]` line, fix the identified files, and re-run the script until it exits 0.

### Step 3 — Deploy

```bash
cd /d <stack_folder> && cdk.cmd deploy --toolkit-stack-name CDKToolkitDemo --require-approval never --ci
```

Wait for the command to exit naturally. Do not interrupt it. Do not run it a second time while it is running.

If deploy fails, run this to read the CloudFormation failure events:
...
Read the failure reason, fix the stack file, re-validate, then re-deploy.

### Step 4 — Confirm and print teardown command

After a successful deploy, print:
- The stack name
- The AWS Console CloudFormation URL for the stack
- The teardown command:
  ```bash
  cd /d <stack_folder> && cdk.cmd destroy --toolkit-stack-name CDKToolkitDemo --force
  ```