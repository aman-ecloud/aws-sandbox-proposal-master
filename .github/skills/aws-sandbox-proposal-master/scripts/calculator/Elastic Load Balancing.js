/**
 * Elastic Load Balancing - AWS Pricing Calculator Script
 *
 * Service name  : Elastic Load Balancing
...
(async function configureElasticLoadBalancing(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB Application Load Balancer baseline',

    // Choose a Region | PRICING IMPACT: true
    region: params?.region ?? 'US East (N. Virginia)',

    // Load balancer type | PRICING IMPACT: true
    // options: 'Application Load Balancer', 'Network Load Balancer', 'Gateway Load Balancer'
    loadBalancerType: params?.loadBalancerType ?? 'Application Load Balancer',

    // Number of load balancers | PRICING IMPACT: true
    numberOfLoadBalancers: params?.numberOfLoadBalancers ?? 1,

    // Hours per month | PRICING IMPACT: true
    hoursPerMonth: params?.hoursPerMonth ?? 730,

    // Processed bytes per hour (GB) | PRICING IMPACT: true
    processedBytesGbPerHour: params?.processedBytesGbPerHour ?? 1,

    // Number of rules | PRICING IMPACT: true
    numberOfRules: params?.numberOfRules ?? 10,

    // New connections per second | PRICING IMPACT: true
    newConnectionsPerSecond: params?.newConnectionsPerSecond ?? 100,
  };

  console.log('[Elastic Load Balancing] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Elastic Load Balancing] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!option) { console.warn('[Elastic Load Balancing] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Elastic Load Balancing] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Elastic Load Balancing');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Elastic Load Balancing'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Elastic Load Balancing] Configure button not found');
      return;
    }
  }
  ...
  await setFieldByAria('Processed bytes', config.processedBytesGbPerHour);
  await setFieldByAria('Number of rules', config.numberOfRules);
  await setFieldByAria('connections per second', config.newConnectionsPerSecond);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Elastic Load Balancing] Saved successfully!');
  } else {
    console.warn('[Elastic Load Balancing] Save and add service button not found');
  }
  ...
  // numberOfLoadBalancers: 1,
  // hoursPerMonth: 730,
  // processedBytesGbPerHour: 1,
  // numberOfRules: 10,
