/**
 * Amazon Route 53 - AWS Pricing Calculator Script
 *
 * Service name  : Amazon Route 53
 * Configure URL : https://calculator.aws/#/addService (resolved at runtime)
 *
 * Note: Route 53 is a global service — no region selector on the form.
 *
 * Usage (AI agent):
...
(async function configureAmazonRoute53(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB Route 53 DNS baseline',

    // Number of hosted zones | PRICING IMPACT: true
    hostedZones: params?.hostedZones ?? 5,

    // DNS queries (millions/month) | PRICING IMPACT: true
    dnsQueriesMillions: params?.dnsQueriesMillions ?? 10,

    // Number of health checks | PRICING IMPACT: true
    healthChecks: params?.healthChecks ?? 5,

    // Traffic flow policies | PRICING IMPACT: true
    trafficPolicies: params?.trafficPolicies ?? 1,
  };

  console.log('[Amazon Route 53] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Amazon Route 53] Field not found:', labelText); return; }
    await jitter();
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon Route 53] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon Route 53');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon Route 53'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon Route 53] Configure button not found');
      return;
    }
  }
  ...
  await setFieldByAria('Hosted zones', config.hostedZones);
  await setFieldByAria('DNS queries', config.dnsQueriesMillions);
  await setFieldByAria('health check', config.healthChecks);
  await setFieldByAria('Traffic flow', config.trafficPolicies);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon Route 53] Saved successfully!');
  } else {
    console.warn('[Amazon Route 53] Save and add service button not found');
  }

})({
  // Override defaults here. All keys are optional — omit to keep script default.
  // hostedZones: 5,
  // dnsQueriesMillions: 10,
  // healthChecks: 5,
  // trafficPolicies: 1,
});
