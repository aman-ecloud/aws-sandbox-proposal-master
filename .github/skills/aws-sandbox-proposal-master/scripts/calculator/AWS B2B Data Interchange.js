/**
 * AWS B2B Data Interchange - AWS Pricing Calculator Script
 *
 * Service name  : AWS B2B Data Interchange
 * Configure URL : https://calculator.aws/#/createCalculator/b2b-data-interchange
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('AWS B2B Data Interchange.js', 'utf8'));
 *
...
(async function configureB2BDataInterchange(params) {
  ...
  const config = {
    ...
    numberOfPartnerships: params?.numberOfPartnerships ?? '10',  // non-pricing metadata
    numberOfTransformationStepsPerMonth: params?.numberOfTransformationStepsPerMonth ?? '10',  // non-pricing metadata
  };

  console.log('[AWS B2B Data Interchange] Starting with config:', config);
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'AWS B2B Data Interchange');
      await wait(2200);
    }

    const configBtn = [...document.querySelectorAll('button')].find(b => {
      const aria = b.getAttribute('aria-label') || '';
      return aria === 'Configure AWS B2B Data Interchange' || ((b.textContent || '').trim() === 'Configure' && (b.closest('li,div')?.textContent || '').includes('AWS B2B Data Interchange'));
    });
    if (configBtn) {
      ...
      console.warn('[AWS B2B Data Interchange] Configure button not found');
      return;
    }
  }
  ...
  await jitter(80, 220);
  const numberOfPartnershipsEl = findField('Number of partnerships');
  if (numberOfPartnershipsEl) {
    setInputValue(numberOfPartnershipsEl, config.numberOfPartnerships);
    await jitter(100, 280);
  }

  await jitter(80, 220);
  const numberOfTransformationStepsPerMonthEl = findField('Number of transformation steps per month');
  if (numberOfTransformationStepsPerMonthEl) {
    setInputValue(numberOfTransformationStepsPerMonthEl, config.numberOfTransformationStepsPerMonth);
    await jitter(100, 280);
  }
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[AWS B2B Data Interchange] Saved successfully!');
  } else {
    console.warn('[AWS B2B Data Interchange] Save and add service button not found');
  }
