/**
 * Amazon Cognito - AWS Pricing Calculator Script
 *
...
(async function configureAmazonCognito(params) {

  const config = {
    // Region | PRICING IMPACT: true
    region: params?.region ?? 'Africa (Cape Town)',

    // Number of monthly active users (MAU) | PRICING IMPACT: true
    monthlyActiveUsers: params?.monthlyActiveUsers ?? 5000,
  };

  console.log('[Amazon Cognito] Starting with config:', config);
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon Cognito] waitForElement timed out:', selector);
    return null;
  }
  ...
  async function setFieldByAria(text, value, index = 0) {
    ...
    if (!el) { console.warn('[Amazon Cognito] Field not found:', text, 'index', index); return; }
    await jitter();
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[Amazon Cognito] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[Amazon Cognito] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[Amazon Cognito] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[Amazon Cognito] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[Amazon Cognito] Region set to:', value);
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon Cognito');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' &&
                 b.closest('li, article')?.textContent?.includes('Amazon Cognito'));
    if (configBtn) {
      scrollTo(configBtn); await jitter(250, 600); configBtn.click(); await wait(5000);
    } else {
      console.warn('[Amazon Cognito] Configure button not found'); return;
    }
  }
  ...
  await setFieldByAria('monthly active users', config.monthlyActiveUsers);
  await setFieldByAria('MAU', config.monthlyActiveUsers);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon Cognito] Saved successfully!');
  } else {
    console.warn('[Amazon Cognito] Save and add service button not found');
  }
