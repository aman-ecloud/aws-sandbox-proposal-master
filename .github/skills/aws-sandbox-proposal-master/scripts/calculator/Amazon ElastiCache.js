/**
 * Amazon ElastiCache - AWS Pricing Calculator Script
 *
...
(async function configureAmazonElastiCache(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB ElastiCache Redis baseline',

    // Choose a Region | PRICING IMPACT: true
    region: params?.region ?? 'US East (N. Virginia)',

    // Engine | PRICING IMPACT: true
    // options: 'Redis', 'Memcached', 'Valkey'
    engine: params?.engine ?? 'Redis',

    // Node type | PRICING IMPACT: true
    // options: 'cache.t3.micro', 'cache.t3.small', 'cache.t3.medium', 'cache.r6g.large'
    nodeType: params?.nodeType ?? 'cache.t3.medium',

    // Number of nodes | PRICING IMPACT: true
    numberOfNodes: params?.numberOfNodes ?? 2,

    // Utilization (hours/month) | PRICING IMPACT: true
    utilizationHours: params?.utilizationHours ?? 730,
  };

  console.log('[Amazon ElastiCache] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Amazon ElastiCache] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!btn) { console.warn('[Amazon ElastiCache] Dropdown not found:', labelText); return; }
    scrollTo(btn);
    await jitter();
    btn.click();
    await wait(600);
    ...
    if (!option) { console.warn('[Amazon ElastiCache] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[Amazon ElastiCache] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[Amazon ElastiCache] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[Amazon ElastiCache] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[Amazon ElastiCache] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[Amazon ElastiCache] Region set to:', value);
  }
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon ElastiCache] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon ElastiCache');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon ElastiCache'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon ElastiCache] Configure button not found');
      return;
    }
  }
  ...
  await selectDropdown('Engine', config.engine);
  await selectDropdown('Node type', config.nodeType);
  await setFieldByAria('Number of nodes', config.numberOfNodes);
  await setFieldByAria('Utilization', config.utilizationHours);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon ElastiCache] Saved successfully!');
  } else {
    console.warn('[Amazon ElastiCache] Save and add service button not found');
  }
