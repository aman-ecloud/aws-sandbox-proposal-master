/**
 * AWS Secrets Manager - AWS Pricing Calculator Script
 *
...
(async function configureAWSSecretsManager(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB Secrets Manager baseline',

    // Choose a Region | PRICING IMPACT: true
    region: params?.region ?? 'US East (N. Virginia)',

    // Number of secrets | PRICING IMPACT: true
    numberOfSecrets: params?.numberOfSecrets ?? 20,

    // API calls (thousands/month) | PRICING IMPACT: true
    apiCallsThousands: params?.apiCallsThousands ?? 100,
  };

  console.log('[AWS Secrets Manager] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[AWS Secrets Manager] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!btn) { console.warn('[AWS Secrets Manager] Dropdown not found:', labelText); return; }
    scrollTo(btn);
    await jitter();
    btn.click();
    await wait(600);
    ...
    if (!option) { console.warn('[AWS Secrets Manager] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[AWS Secrets Manager] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[AWS Secrets Manager] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[AWS Secrets Manager] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[AWS Secrets Manager] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[AWS Secrets Manager] Region set to:', value);
  }
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[AWS Secrets Manager] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'AWS Secrets Manager');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Secrets Manager'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[AWS Secrets Manager] Configure button not found');
      return;
    }
  }
  ...
  await setFieldByAria('Number of secrets', config.numberOfSecrets);
  await setFieldByAria('API calls', config.apiCallsThousands);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[AWS Secrets Manager] Saved successfully!');
  } else {
    console.warn('[AWS Secrets Manager] Save and add service button not found');
  }
