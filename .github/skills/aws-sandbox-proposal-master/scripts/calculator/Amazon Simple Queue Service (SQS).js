/**
 * Amazon Simple Queue Service (SQS) - AWS Pricing Calculator Script
 *
 * Service name  : Amazon Simple Queue Service (SQS)
...
(async function configureAmazonSQS(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB SQS message queuing baseline',

    // Choose a Region | PRICING IMPACT: true
    region: params?.region ?? 'US East (N. Virginia)',

    // Standard queue requests (millions/month) | PRICING IMPACT: true
    standardRequestsMillions: params?.standardRequestsMillions ?? 10,

    // FIFO queue requests (millions/month) | PRICING IMPACT: true
    fifoRequestsMillions: params?.fifoRequestsMillions ?? 0,

    // Data transfer out (GB/month) | PRICING IMPACT: true
    dataTransferOutGb: params?.dataTransferOutGb ?? 10,
  };

  console.log('[Amazon Simple Queue Service (SQS)] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Amazon SQS] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!option) { console.warn('[Amazon SQS] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[Amazon Simple Queue Service (SQS)] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[Amazon Simple Queue Service (SQS)] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[Amazon Simple Queue Service (SQS)] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[Amazon Simple Queue Service (SQS)] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[Amazon Simple Queue Service (SQS)] Region set to:', value);
  }
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon SQS] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon Simple Queue Service (SQS)');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Simple Queue Service'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon SQS] Configure button not found');
      return;
    }
  }
  ...
  await setFieldByAria('Standard queue', config.standardRequestsMillions);
  await setFieldByAria('FIFO queue', config.fifoRequestsMillions);
  await setFieldByAria('Data transfer out', config.dataTransferOutGb);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon Simple Queue Service (SQS)] Saved successfully!');
  } else {
    console.warn('[Amazon SQS] Save and add service button not found');
  }

})({
  // Override defaults here. All keys are optional — omit to keep script default.
  // region: 'US East (N. Virginia)',
  // standardRequestsMillions: 10,
  // fifoRequestsMillions: 0,
