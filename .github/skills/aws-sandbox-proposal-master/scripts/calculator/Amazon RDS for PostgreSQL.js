/**
 * Amazon RDS for PostgreSQL - AWS Pricing Calculator Script
 *
...
(async function configureAmazonRDSForPostgreSQL(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB RDS PostgreSQL baseline',
  ...
  console.log('[Amazon RDS for PostgreSQL] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Amazon RDS for PostgreSQL] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!btn) { console.warn('[Amazon RDS for PostgreSQL] Dropdown not found:', labelText); return; }
    scrollTo(btn);
    await jitter();
    btn.click();
    await wait(600);
    ...
    if (!option) { console.warn('[Amazon RDS for PostgreSQL] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[Amazon RDS for PostgreSQL] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[Amazon RDS for PostgreSQL] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[Amazon RDS for PostgreSQL] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[Amazon RDS for PostgreSQL] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[Amazon RDS for PostgreSQL] Region set to:', value);
  }
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon RDS for PostgreSQL] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon RDS for PostgreSQL');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('RDS for PostgreSQL'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon RDS for PostgreSQL] Configure button not found');
      return;
    }
  }
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon RDS for PostgreSQL] Saved successfully!');
  } else {
    console.warn('[Amazon RDS for PostgreSQL] Save and add service button not found');
  }
