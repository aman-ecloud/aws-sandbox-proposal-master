/**
 * Amazon Virtual Private Cloud (VPC) - AWS Pricing Calculator Script
 *
...
(async function configureAmazonVPC(params) {

  const config = {
    // Description - optional | PRICING IMPACT: false
    description: params?.description ?? 'SMB VPC networking baseline',

    // Choose a Region | PRICING IMPACT: true
    region: params?.region ?? 'US East (N. Virginia)',

    // NAT Gateway — number of NAT Gateways | PRICING IMPACT: true
    natGateways: params?.natGateways ?? 1,

    // NAT Gateway — data processed per gateway (GB/month) | PRICING IMPACT: true
    natGatewayDataProcessedGb: params?.natGatewayDataProcessedGb ?? 100,

    // VPN connections | PRICING IMPACT: true
    vpnConnections: params?.vpnConnections ?? 1,

    // VPN connection hours/month | PRICING IMPACT: true
    vpnConnectionHours: params?.vpnConnectionHours ?? 730,

    // PrivateLink endpoints | PRICING IMPACT: true
    privateLinkEndpoints: params?.privateLinkEndpoints ?? 2,
  };

  console.log('[Amazon Virtual Private Cloud (VPC)] Starting with config:', config);
  ...
  async function setFieldByAria(labelText, value, index = 0) {
    ...
    const el = all[index];
    if (!el) { console.warn('[Amazon VPC] Field not found:', labelText); return; }
    await jitter();
  ...
  async function selectDropdown(labelText, value, index = 0) {
    ...
    if (!btn) { console.warn('[Amazon VPC] Dropdown not found:', labelText); return; }
    scrollTo(btn);
    await jitter();
    btn.click();
    await wait(600);
    ...
    if (!option) { console.warn('[Amazon VPC] Option not found:', value); return; }
    scrollTo(option);
    await jitter();
    option.click();
    await jitter(100, 300);
  ...
  async function selectRegion(value) {
    ...
    if (!regionBtn) { console.warn('[Amazon Virtual Private Cloud (VPC)] Region dialog button not found'); return; }
    scrollTo(regionBtn);
    await jitter();
    ...
    if (searchInput) {
      ...
      console.warn('[Amazon Virtual Private Cloud (VPC)] combobox search input not found');
    }
    const option = [...document.querySelectorAll('[role="option"]')]
      .find(o => o.textContent.trim().includes(value));
    if (!option) { console.warn('[Amazon Virtual Private Cloud (VPC)] Region option not found:', value); return; }
    scrollTo(option);
    await jitter();
    ...
    if (stillOpen) {
      console.warn('[Amazon Virtual Private Cloud (VPC)] Region dialog still open after click; retrying');
      option.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true }));
      option.dispatchEvent(new MouseEvent('mousedown',  { bubbles: true, cancelable: true }));
      option.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
      option.dispatchEvent(new MouseEvent('mouseup',    { bubbles: true }));
    ...
    console.log('[Amazon Virtual Private Cloud (VPC)] Region set to:', value);
  }
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon VPC] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'Amazon Virtual Private Cloud (VPC)');
      await wait(1800);
    }

    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Virtual Private Cloud'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon VPC] Configure button not found');
      return;
    }
  }
  ...
  await setFieldByAria('Number of NAT Gateways', config.natGateways);
  await setFieldByAria('Data processed per NAT Gateway', config.natGatewayDataProcessedGb);
  await setFieldByAria('VPN connections', config.vpnConnections);
  await setFieldByAria('VPN connection hours', config.vpnConnectionHours);
  await setFieldByAria('PrivateLink', config.privateLinkEndpoints);
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[Amazon Virtual Private Cloud (VPC)] Saved successfully!');
  } else {
    console.warn('[Amazon VPC] Save and add service button not found');
  }
  ...
  // natGateways: 1,
  // natGatewayDataProcessedGb: 100,
  // vpnConnections: 1,
  // vpnConnectionHours: 730,
  // privateLinkEndpoints: 2,
});
