 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('Amazon Simple Notification Service (SNS).js', 'utf8'));
 *
...
(async function configureAmazonSimpleNotificationServiceSNS(params) {
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon Simple Notification Service (SNS)] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon Simple Notification Service (SNS)'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon Simple Notification Service (SNS)] Configure button not found');
      return;
    }
  }
  ...
  if (saveBtn) {
    ...
    console.warn('[Amazon Simple Notification Service (SNS)] Save and add service button not found');
  }
