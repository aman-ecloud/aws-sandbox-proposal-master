 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('Amazon RDS for PostgreSQL.js', 'utf8'));
 *
...
(async function configureAmazonRDSForPostgreSQL(params) {
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon RDS for PostgreSQL'));
    if (configBtn) {
      scrollTo(configBtn);
