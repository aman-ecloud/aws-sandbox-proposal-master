 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('AWS Secrets Manager.js', 'utf8'));
 *
...
(async function configureAWSSecretsManager(params) {
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('AWS Secrets Manager'));
    if (configBtn) {
      scrollTo(configBtn);
