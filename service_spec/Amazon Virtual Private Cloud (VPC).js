 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('Amazon Virtual Private Cloud (VPC).js', 'utf8'));
 *
...
(async function configureAmazonVirtualPrivateCloudVPC(params) {
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon Virtual Private Cloud (VPC)] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon Virtual Private Cloud (VPC)'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon Virtual Private Cloud (VPC)] Configure button not found');
      return;
    }
  }
  ...
  if (saveBtn) {
    ...
    console.warn('[Amazon Virtual Private Cloud (VPC)] Save and add service button not found');
  }
