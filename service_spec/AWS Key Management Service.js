 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('AWS Key Management Service.js', 'utf8'));
 *
...
(async function configureAWSKeyManagementService(params) {
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[AWS Key Management Service] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('AWS Key Management Service'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[AWS Key Management Service] Configure button not found');
      return;
    }
  }
  ...
  if (saveBtn) {
    ...
    console.warn('[AWS Key Management Service] Save and add service button not found');
  }
