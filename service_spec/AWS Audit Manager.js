/**
 * AWS Audit Manager - AWS Pricing Calculator Script
 *
 * Service name  : AWS Audit Manager
 * Configure URL : https://calculator.aws/#/createCalculator/awsAuditManager
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('AWS Audit Manager.js', 'utf8'));
 *
...
(async function configureAuditManager(params) {
  ...
  const config = {
    ...
    numberOfAccounts: params?.numberOfAccounts ?? '5',  // non-pricing metadata
    resourceType: params?.resourceType ?? 'EC2 instances',  // non-pricing metadata | options: EC2 instances, RDS instances, S3 buckets, VPC subnets, Other resource types
    numberOfResourcesPerAccount: params?.numberOfResourcesPerAccount ?? '100',  // non-pricing metadata
    numberOfConfigurationSnapshotsApiCalls: params?.numberOfConfigurationSnapshotsApiCalls ?? 1,  // pricing driver
    unit: params?.unit ?? 'per month',  // non-pricing metadata | options: per day, per week, per month
    numberOfConfigurationChangesUserActivity: params?.numberOfConfigurationChangesUserActivity ?? 1,  // pricing driver
    numberOfComplianceChecksSecurityHub: params?.numberOfComplianceChecksSecurityHub ?? 1,  // pricing driver
  };

  console.log('[AWS Audit Manager] Starting with config:', config);
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    if (searchBox) {
      ...
      setInputValue(searchBox, 'AWS Audit Manager');
      await wait(2200);
    }

    const configBtn = [...document.querySelectorAll('button')].find(b => {
      const aria = b.getAttribute('aria-label') || '';
      return aria === 'Configure AWS Audit Manager' || ((b.textContent || '').trim() === 'Configure' && (b.closest('li,div')?.textContent || '').includes('AWS Audit Manager'));
    });
    if (configBtn) {
      ...
      console.warn('[AWS Audit Manager] Configure button not found');
      return;
    }
  }
  ...
  await jitter(80, 220);
  const numberOfAccountsEl = findField('Number of accounts');
  if (numberOfAccountsEl) {
    setInputValue(numberOfAccountsEl, config.numberOfAccounts);
    await jitter(100, 280);
  }

  await jitter(80, 220);
  await selectDropdown('Resource type', config.resourceType);

  await jitter(80, 220);
  const numberOfResourcesPerAccountEl = findField('Number of resources per account');
  if (numberOfResourcesPerAccountEl) {
    setInputValue(numberOfResourcesPerAccountEl, config.numberOfResourcesPerAccount);
    await jitter(100, 280);
  }

  await jitter(80, 220);
  const numberOfConfigurationSnapshotsApiCallsEl = findField('Number of configuration snapshots (API calls) Value');
  if (numberOfConfigurationSnapshotsApiCallsEl) {
    setInputValue(numberOfConfigurationSnapshotsApiCallsEl, config.numberOfConfigurationSnapshotsApiCalls);
    await jitter(100, 280);
  }
  ...
  await jitter(80, 220);
  const numberOfConfigurationChangesUserActivityEl = findField('Number of configuration changes/user activity logs (Cloudtrail) Value');
  if (numberOfConfigurationChangesUserActivityEl) {
    setInputValue(numberOfConfigurationChangesUserActivityEl, config.numberOfConfigurationChangesUserActivity);
    await jitter(100, 280);
  }

  await jitter(80, 220);
  const numberOfComplianceChecksSecurityHubEl = findField('Number of compliance checks (Security Hub, Config) Value');
  if (numberOfComplianceChecksSecurityHubEl) {
    setInputValue(numberOfComplianceChecksSecurityHubEl, config.numberOfComplianceChecksSecurityHub);
    await jitter(100, 280);
  }
  ...
  if (saveBtn) {
    ...
    saveBtn.click();
    console.log('[AWS Audit Manager] Saved successfully!');
  } else {
    console.warn('[AWS Audit Manager] Save and add service button not found');
  }
