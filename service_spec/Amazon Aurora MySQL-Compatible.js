 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('Amazon Aurora MySQL-Compatible.js', 'utf8'));
 *
...
(async function configureAmazonAuroraMySQLCompatible(params) {
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon Aurora MySQL-Compatible'));
    if (configBtn) {
      scrollTo(configBtn);
