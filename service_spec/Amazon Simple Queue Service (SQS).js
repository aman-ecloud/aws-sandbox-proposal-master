 ...
 *
 * Usage (AI agent):
 *   await page.evaluate(fs.readFileSync('Amazon Simple Queue Service (SQS).js', 'utf8'));
 *
...
(async function configureAmazonSimpleQueueServiceSQS(params) {
  ...
  async function waitForElement(selector, timeout = 12000) {
    ...
    console.warn('[Amazon Simple Queue Service (SQS)] waitForElement timed out:', selector);
    return null;
  }
  ...
  if (window.location.hash.includes('/addService')) {
    ...
    const configBtn = [...document.querySelectorAll('button')]
      .find(b => b.textContent.trim() === 'Configure' && b.closest('li, article')?.textContent?.includes('Amazon Simple Queue Service (SQS)'));
    if (configBtn) {
      scrollTo(configBtn);
      ...
      console.warn('[Amazon Simple Queue Service (SQS)] Configure button not found');
      return;
    }
  }
  ...
  if (saveBtn) {
    ...
    console.warn('[Amazon Simple Queue Service (SQS)] Save and add service button not found');
  }
